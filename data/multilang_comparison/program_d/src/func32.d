...
array_t* func32(array_t_param* vars, const ulong PATH0, int loopsFactor) {
   ...
   if (pCounter > 0) {
      ...
      array59 = cast(array_t*)malloc(array_t.sizeof);
      array59.size = 538;
      array59.refC = 1;
      array59.id = 59;
      array59.data = cast(uint*)malloc(array59.size*uint.sizeof);
      memset(array59.data, 0, array59.size*uint.sizeof);
      DEBUG_NEW(array59.id);
   }
   uint loop19 = 0;
   uint loopLimit19 = (50)/4 + 1;
   for(; loop19 < loopLimit19; loop19++) {
      if(PATH0 & 1) {
         ...
         if (pCounter > 0) {
            ...
            array60 = cast(array_t*)malloc(array_t.sizeof);
            array60.size = 606;
            array60.refC = 1;
            array60.id = 60;
            array60.data = cast(uint*)malloc(array60.size*uint.sizeof);
            memset(array60.data, 0, array60.size*uint.sizeof);
            DEBUG_NEW(array60.id);
         }
         uint loop20 = 0;
         uint loopLimit20 = (50)/5 + 1;
      ...
      if (pCounter > 0) {
         ...
         array62 = cast(array_t*)malloc(array_t.sizeof);
         array62.size = 369;
         array62.refC = 1;
         array62.id = 62;
         array62.data = cast(uint*)malloc(array62.size*uint.sizeof);
         memset(array62.data, 0, array62.size*uint.sizeof);
