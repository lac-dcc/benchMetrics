...
array_t* func22(array_t_param* vars, const ulong PATH0, int loopsFactor) {
   ...
   if (pCounter > 0) {
      ...
      array35 = cast(array_t*)malloc(array_t.sizeof);
      array35.size = 567;
      array35.refC = 1;
      array35.id = 35;
      array35.data = cast(uint*)malloc(array35.size*uint.sizeof);
      memset(array35.data, 0, array35.size*uint.sizeof);
      DEBUG_NEW(array35.id);
   }
   uint loop12 = 0;
   uint loopLimit12 = (50)/4 + 1;
   for(; loop12 < loopLimit12; loop12++) {
      if(PATH0 & 1) {
         ...
         if (pCounter > 0) {
            ...
            array36 = cast(array_t*)malloc(array_t.sizeof);
            array36.size = 856;
            array36.refC = 1;
            array36.id = 36;
            array36.data = cast(uint*)malloc(array36.size*uint.sizeof);
            memset(array36.data, 0, array36.size*uint.sizeof);
            DEBUG_NEW(array36.id);
         }
         uint loop13 = 0;
         uint loopLimit13 = (50)/5 + 1;
      ...
      if (pCounter > 0) {
         ...
         array39 = cast(array_t*)malloc(array_t.sizeof);
         array39.size = 624;
         array39.refC = 1;
         array39.id = 39;
         array39.data = cast(uint*)malloc(array39.size*uint.sizeof);
         memset(array39.data, 0, array39.size*uint.sizeof);
      ...
      if(PATH0 & 2) {
         ...
         if (pCounter > 0) {
            ...
            array41 = cast(array_t*)malloc(array_t.sizeof);
            array41.size = 732;
            array41.refC = 1;
            array41.id = 41;
            array41.data = cast(uint*)malloc(array41.size*uint.sizeof);
            memset(array41.data, 0, array41.size*uint.sizeof);
            DEBUG_NEW(array41.id);
         }
         uint loop14 = 0;
         uint loopLimit14 = (50)/5 + 1;
