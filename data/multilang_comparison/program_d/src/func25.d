...
array_t* func25(array_t_param* vars, const ulong PATH0, int loopsFactor) {
   size_t pCounter = vars.size;
   uint loop22 = 0;
   uint loopLimit22 = (50)/3 + 1;
   for(; loop22 < loopLimit22; loop22++) {
      if(PATH0 & 1) {
         ...
         if (pCounter > 0) {
            ...
            array69 = cast(array_t*)malloc(array_t.sizeof);
            array69.size = 828;
            array69.refC = 1;
            array69.id = 69;
            array69.data = cast(uint*)malloc(array69.size*uint.sizeof);
            memset(array69.data, 0, array69.size*uint.sizeof);
            DEBUG_NEW(array69.id);
         }
         uint loop23 = 0;
         uint loopLimit23 = (50)/4 + 1;
      ...
      if (pCounter > 0) {
         ...
         array72 = cast(array_t*)malloc(array_t.sizeof);
         array72.size = 810;
         array72.refC = 1;
         array72.id = 72;
         array72.data = cast(uint*)malloc(array72.size*uint.sizeof);
         memset(array72.data, 0, array72.size*uint.sizeof);
      ...
      if(PATH0 & 2) {
         ...
         if (pCounter > 0) {
            ...
            array74 = cast(array_t*)malloc(array_t.sizeof);
            array74.size = 661;
            array74.refC = 1;
            array74.id = 74;
            array74.data = cast(uint*)malloc(array74.size*uint.sizeof);
            memset(array74.data, 0, array74.size*uint.sizeof);
            DEBUG_NEW(array74.id);
         }
         uint loop24 = 0;
         uint loopLimit24 = (50)/4 + 1;
   ...
   if (pCounter > 0) {
      ...
      array76 = cast(array_t*)malloc(array_t.sizeof);
      array76.size = 522;
      array76.refC = 1;
      array76.id = 76;
      array76.data = cast(uint*)malloc(array76.size*uint.sizeof);
      memset(array76.data, 0, array76.size*uint.sizeof);
