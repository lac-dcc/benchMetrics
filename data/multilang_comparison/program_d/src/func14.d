...
array_t* func14(array_t_param* vars, const ulong PATH0, int loopsFactor) {
   ...
   if (pCounter > 0) {
      ...
      array147 = cast(array_t*)malloc(array_t.sizeof);
      array147.size = 504;
      array147.refC = 1;
      array147.id = 147;
      array147.data = cast(uint*)malloc(array147.size*uint.sizeof);
      memset(array147.data, 0, array147.size*uint.sizeof);
      DEBUG_NEW(array147.id);
   }
   uint loop43 = 0;
   uint loopLimit43 = (50)/3 + 1;
   for(; loop43 < loopLimit43; loop43++) {
      if(PATH0 & 1) {
         ...
         if (pCounter > 0) {
            ...
            array148 = cast(array_t*)malloc(array_t.sizeof);
            array148.size = 629;
            array148.refC = 1;
            array148.id = 148;
            array148.data = cast(uint*)malloc(array148.size*uint.sizeof);
            memset(array148.data, 0, array148.size*uint.sizeof);
            DEBUG_NEW(array148.id);
         }
         uint loop44 = 0;
         uint loopLimit44 = (50)/4 + 1;
         ...
         array_t* array149 = func28(&params0, get_path(), loopsFactor);
         ...
         params0.data = cast(array_t**)malloc(params0.size*(array_t*).sizeof);
         params0.data[0] = array147;
         array_t* array150 = func22(&params0, get_path(), loopsFactor);
      ...
      if (pCounter > 0) {
         ...
         array151 = cast(array_t*)malloc(array_t.sizeof);
         array151.size = 900;
         array151.refC = 1;
         array151.id = 151;
         array151.data = cast(uint*)malloc(array151.size*uint.sizeof);
         memset(array151.data, 0, array151.size*uint.sizeof);
      ...
      if(PATH0 & 2) {
         ...
         if (pCounter > 0) {
            ...
            array153 = cast(array_t*)malloc(array_t.sizeof);
            array153.size = 949;
            array153.refC = 1;
            array153.id = 153;
            array153.data = cast(uint*)malloc(array153.size*uint.sizeof);
            memset(array153.data, 0, array153.size*uint.sizeof);
            DEBUG_NEW(array153.id);
         }
         uint loop45 = 0;
         uint loopLimit45 = (50)/4 + 1;
         ...
         array_t* array154 = func36(&params1, get_path(), loopsFactor);
         ...
         params1.data[1] = array151;
         params1.data[2] = array152;
         array_t* array155 = func32(&params1, get_path(), loopsFactor);
