...
array_t* func1(array_t_param* vars, const ulong PATH0, int loopsFactor) {
   ...
   if (pCounter > 0) {
      ...
      array193 = cast(array_t*)malloc(array_t.sizeof);
      array193.size = 462;
      array193.refC = 1;
      array193.id = 193;
      array193.data = cast(uint*)malloc(array193.size*uint.sizeof);
      memset(array193.data, 0, array193.size*uint.sizeof);
   ...
   if(PATH0 & 1) {
      ...
      if (pCounter > 0) {
         ...
         array197 = cast(array_t*)malloc(array_t.sizeof);
         array197.size = 951;
         array197.refC = 1;
         array197.id = 197;
         array197.data = cast(uint*)malloc(array197.size*uint.sizeof);
         memset(array197.data, 0, array197.size*uint.sizeof);
         DEBUG_NEW(array197.id);
      }
      uint loop57 = 0;
      uint loopLimit57 = (50)/2 + 1;
