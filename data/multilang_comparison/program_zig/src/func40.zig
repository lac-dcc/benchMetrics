...
pub fn func40(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array10: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array10 = vars.data[pCounter];
       array10.refC += 1;
   } else {
       array10 = try allocator.create(Array);
       array10.*.size = 42;
       array10.*.refC = 1;
       array10.*.id = 10;
       array10.*.data = try allocator.alloc(u32, array10.*.size);
   }
   for (0..array10.size) |i| {
       array10.data[i] -= 1;
   }
   for (0..array10.size) |i| {
