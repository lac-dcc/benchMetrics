...
pub fn func23(vars: *ArrayParam) !*Array {
   ...
   _ = &pCounter;
   var array45: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array45 = vars.data[pCounter];
       array45.refC += 1;
   } else {
       array45 = try allocator.create(Array);
       array45.*.size = 555;
       array45.*.refC = 1;
       array45.*.id = 45;
       array45.*.data = try allocator.alloc(u32, array45.*.size);
   }
   ...
   const array46 = try func28(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop15: usize = 0;
   const loopLimit15: usize = (50)/4 + 1;
   while (loop15 < loopLimit15) {
      for (0..array45.size) |i| {
          array45.data[i] += 1;
      }
      loop15 += 1;
   }
