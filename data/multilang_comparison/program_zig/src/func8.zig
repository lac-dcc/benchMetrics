...
pub fn func8(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array170: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array170 = vars.data[pCounter];
          array170.refC += 1;
      } else {
          array170 = try allocator.create(Array);
          array170.*.size = 627;
          array170.*.refC = 1;
          array170.*.id = 170;
          array170.*.data = try allocator.alloc(u32, array170.*.size);
      }
      var loop50: usize = 0;
      const loopLimit50: usize = (50)/2 + 1;
      while (loop50 < loopLimit50) {
         for (0..array170.size) |i| {
             array170.data[i] -= 1;
         }
         for (0..array170.size) |i| {
      ...
      const array171 = try func18(&params0, try get_path());
      ...
      const array172 = try func14(&params0, try get_path());
      defer allocator.free(params0.data);
      array172.refC -= 1;
      if (array172.refC == 0) {
          allocator.free(array172.data);
          allocator.destroy(array172);
      }
   }
   var array173: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array173 = vars.data[pCounter];
       array173.refC += 1;
   } else {
       array173 = try allocator.create(Array);
       array173.*.size = 479;
       array173.*.refC = 1;
       array173.*.id = 173;
       array173.*.data = try allocator.alloc(u32, array173.*.size);
   }
   return array173;
}
