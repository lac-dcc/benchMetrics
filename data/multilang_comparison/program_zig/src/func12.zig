...
pub fn func12(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array66: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array66 = vars.data[pCounter];
          array66.refC += 1;
      } else {
          array66 = try allocator.create(Array);
          array66.*.size = 786;
          array66.*.refC = 1;
          array66.*.id = 66;
          array66.*.data = try allocator.alloc(u32, array66.*.size);
      }
      var loop21: usize = 0;
      const loopLimit21: usize = (50)/3 + 1;
      ...
      const array67 = try func24(&params0, try get_path());
      defer allocator.free(params0.data);
      array67.refC -= 1;
      if (array67.refC == 0) {
          allocator.free(array67.data);
          allocator.destroy(array67);
      }
      array66.refC -= 1;
      if (array66.refC == 0) {
      ...
      const array90 = try func16(&params0, try get_path());
      defer allocator.free(params0.data);
      array90.refC -= 1;
      if (array90.refC == 0) {
