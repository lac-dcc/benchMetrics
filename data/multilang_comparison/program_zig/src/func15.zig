...
pub fn func15(vars: *ArrayParam) !*Array {
   ...
   const array178 = try func18(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop51: usize = 0;
   const loopLimit51: usize = (50)/2 + 1;
   while (loop51 < loopLimit51) {
      for (0..array177.size) |i| {
          array177.data[i] += 1;
      }
      loop51 += 1;
   }
   ...
   array178.refC -= 1;
   if (array178.refC == 0) {
       allocator.free(array178.data);
       allocator.destroy(array178);
   }
   return array177;
}
