...
pub fn func38(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array52: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array52 = vars.data[pCounter];
          array52.refC += 1;
      } else {
          array52 = try allocator.create(Array);
          array52.*.size = 275;
          array52.*.refC = 1;
          array52.*.id = 52;
          array52.*.data = try allocator.alloc(u32, array52.*.size);
      }
      var loop18: usize = 0;
      const loopLimit18: usize = (50)/4 + 1;
      while (loop18 < loopLimit18) {
         for (0..array52.size) |i| {
             array52.data[i] -= 1;
         }
         for (0..array52.size) |i| {
      ...
      array52.refC -= 1;
      if (array52.refC == 0) {
          allocator.free(array52.data);
          allocator.destroy(array52);
      }
      ...
      const array53 = try func40(&params0);
      defer allocator.free(params0.data);
      array53.refC -= 1;
      if (array53.refC == 0) {
          allocator.free(array53.data);
          allocator.destroy(array53);
      }
   }
   var array54: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array54 = vars.data[pCounter];
       array54.refC += 1;
   } else {
       array54 = try allocator.create(Array);
       array54.*.size = 29;
       array54.*.refC = 1;
       array54.*.id = 54;
       array54.*.data = try allocator.alloc(u32, array54.*.size);
   }
   return array54;
}
