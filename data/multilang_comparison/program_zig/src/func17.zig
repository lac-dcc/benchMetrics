...
pub fn func17(vars: *ArrayParam) !*Array {
   ...
   _ = &pCounter;
   var array104: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array104 = vars.data[pCounter];
       array104.refC += 1;
   } else {
       array104 = try allocator.create(Array);
       array104.*.size = 269;
       array104.*.refC = 1;
       array104.*.id = 104;
       array104.*.data = try allocator.alloc(u32, array104.*.size);
   }
   ...
   const array105 = try func24(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop31: usize = 0;
   const loopLimit31: usize = (50)/3 + 1;
   while (loop31 < loopLimit31) {
      for (0..array104.size) |i| {
          array104.data[i] += 1;
      }
      loop31 += 1;
   }
   ...
   array105.refC -= 1;
   if (array105.refC == 0) {
       allocator.free(array105.data);
       allocator.destroy(array105);
   }
   return array104;
}
