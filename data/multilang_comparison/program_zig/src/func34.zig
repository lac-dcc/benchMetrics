...
pub fn func34(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array25: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array25 = vars.data[pCounter];
       array25.refC += 1;
   } else {
       array25 = try allocator.create(Array);
       array25.*.size = 368;
       array25.*.refC = 1;
       array25.*.id = 25;
       array25.*.data = try allocator.alloc(u32, array25.*.size);
   }
   var loop9: usize = 0;
   const loopLimit9: usize = (50)/4 + 1;
   while (loop9 < loopLimit9) {
      for (0..array25.size) |i| {
