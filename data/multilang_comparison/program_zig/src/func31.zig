...
pub fn func31(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   const path0 = _path0;
   _ = &path0;
   var array28: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array28 = vars.data[pCounter];
       array28.refC += 1;
   } else {
       array28 = try allocator.create(Array);
       array28.*.size = 97;
       array28.*.refC = 1;
       array28.*.id = 28;
       array28.*.data = try allocator.alloc(u32, array28.*.size);
   }
   ...
   const array29 = try func35(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array32: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array32 = vars.data[pCounter];
          array32.refC += 1;
      } else {
          array32 = try allocator.create(Array);
          array32.*.size = 444;
          array32.*.refC = 1;
          array32.*.id = 32;
          array32.*.data = try allocator.alloc(u32, array32.*.size);
      }
      var loop11: usize = 0;
      const loopLimit11: usize = (50)/4 + 1;
      while (loop11 < loopLimit11) {
         for (0..array32.size) |i| {
             array32.data[i] -= 1;
         }
         for (0..array29.size) |i| {
      ...
      array32.refC -= 1;
      if (array32.refC == 0) {
          allocator.free(array32.data);
          allocator.destroy(array32);
      }
      ...
      const array33 = try func40(&params1);
      defer allocator.free(params1.data);
      array33.refC -= 1;
      if (array33.refC == 0) {
          allocator.free(array33.data);
          allocator.destroy(array33);
      }
   }
   array28.refC -= 1;
   if (array28.refC == 0) {
       allocator.free(array28.data);
       allocator.destroy(array28);
   }
   return array29;
}
