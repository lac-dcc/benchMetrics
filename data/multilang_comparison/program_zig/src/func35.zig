...
pub fn func35(vars: *ArrayParam) !*Array {
   ...
   _ = &pCounter;
   var array30: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array30 = vars.data[pCounter];
       array30.refC += 1;
   } else {
       array30 = try allocator.create(Array);
       array30.*.size = 756;
       array30.*.refC = 1;
       array30.*.id = 30;
       array30.*.data = try allocator.alloc(u32, array30.*.size);
   }
   ...
   const array31 = try func42(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop10: usize = 0;
   const loopLimit10: usize = (50)/4 + 1;
   while (loop10 < loopLimit10) {
      for (0..array30.size) |i| {
          array30.data[i] += 1;
      }
      loop10 += 1;
   }
   for (0..array31.size) |i| {
   ...
   array30.refC -= 1;
   if (array30.refC == 0) {
       allocator.free(array30.data);
       allocator.destroy(array30);
   }
   return array31;
}
