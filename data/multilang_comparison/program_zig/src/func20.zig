...
pub fn func20(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array136: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array136 = vars.data[pCounter];
          array136.refC += 1;
      } else {
          array136 = try allocator.create(Array);
          array136.*.size = 460;
          array136.*.refC = 1;
          array136.*.id = 136;
          array136.*.data = try allocator.alloc(u32, array136.*.size);
      }
      var loop41: usize = 0;
      const loopLimit41: usize = (50)/3 + 1;
      while (loop41 < loopLimit41) {
         for (0..array136.size) |i| {
             array136.data[i] -= 1;
         }
         for (0..array136.size) |i| {
      ...
      const array137 = try func28(&params0, try get_path());
      defer allocator.free(params0.data);
      array137.refC -= 1;
      if (array137.refC == 0) {
          allocator.free(array137.data);
          allocator.destroy(array137);
      }
      array136.refC -= 1;
      if (array136.refC == 0) {
      ...
      const array138 = try func22(&params0, try get_path());
      defer allocator.free(params0.data);
      array138.refC -= 1;
      if (array138.refC == 0) {
          allocator.free(array138.data);
          allocator.destroy(array138);
      }
   }
