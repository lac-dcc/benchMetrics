...
pub fn func39(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array56: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array56 = vars.data[pCounter];
       array56.refC += 1;
   } else {
       array56 = try allocator.create(Array);
       array56.*.size = 143;
       array56.*.refC = 1;
       array56.*.id = 56;
       array56.*.data = try allocator.alloc(u32, array56.*.size);
   }
   ...
   const array57 = try func41(&params0);
