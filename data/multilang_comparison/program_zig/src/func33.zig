...
pub fn func33(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array85: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array85 = vars.data[pCounter];
       array85.refC += 1;
   } else {
       array85 = try allocator.create(Array);
       array85.*.size = 163;
       array85.*.refC = 1;
       array85.*.id = 85;
       array85.*.data = try allocator.alloc(u32, array85.*.size);
   }
   ...
   const array86 = try func36(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop26: usize = 0;
   const loopLimit26: usize = (50)/3 + 1;
   ...
   array85.refC -= 1;
   if (array85.refC == 0) {
       allocator.free(array85.data);
       allocator.destroy(array85);
   }
   return array86;
}
