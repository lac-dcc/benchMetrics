...
pub fn func10(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   if ((path0 & 1) != 0) {
      ...
      const array4 = try func11(&params0, try get_path());
      defer allocator.free(params0.data);
      array4.refC -= 1;
      if (array4.refC == 0) {
          allocator.free(array4.data);
          allocator.destroy(array4);
      }
      ...
      const array65 = try func12(&params0, try get_path());
      defer allocator.free(params0.data);
      array65.refC -= 1;
      if (array65.refC == 0) {
   ...
   const array101 = try func13(&params0, try get_path());
   defer allocator.free(params0.data);
   return array101;
}
