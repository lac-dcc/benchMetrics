...
pub fn func30(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array16: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array16 = vars.data[pCounter];
          array16.refC += 1;
      } else {
          array16 = try allocator.create(Array);
          array16.*.size = 84;
          array16.*.refC = 1;
          array16.*.id = 16;
          array16.*.data = try allocator.alloc(u32, array16.*.size);
      }
      var loop8: usize = 0;
      const loopLimit8: usize = (50)/4 + 1;
      while (loop8 < loopLimit8) {
         for (0..array16.size) |i| {
             array16.data[i] -= 1;
         }
         for (0..array16.size) |i| {
             if (array16.data[i] == 5) {
      ...
      const array17 = try func42(&params0, try get_path());
      defer allocator.free(params0.data);
      array17.refC -= 1;
      if (array17.refC == 0) {
          allocator.free(array17.data);
          allocator.destroy(array17);
      }
      array16.refC -= 1;
      if (array16.refC == 0) {
          allocator.free(array16.data);
          allocator.destroy(array16);
      }
      ...
      const array24 = try func34(&params0);
      defer allocator.free(params0.data);
      array24.refC -= 1;
      if (array24.refC == 0) {
          allocator.free(array24.data);
          allocator.destroy(array24);
      }
   }
   var array26: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array26 = vars.data[pCounter];
       array26.refC += 1;
   } else {
       array26 = try allocator.create(Array);
       array26.*.size = 795;
       array26.*.refC = 1;
       array26.*.id = 26;
       array26.*.data = try allocator.alloc(u32, array26.*.size);
   }
   return array26;
}
