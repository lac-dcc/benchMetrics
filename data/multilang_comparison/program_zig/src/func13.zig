...
pub fn func13(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   const path0 = _path0;
   _ = &path0;
   var array102: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array102 = vars.data[pCounter];
       array102.refC += 1;
   } else {
       array102 = try allocator.create(Array);
       array102.*.size = 984;
       array102.*.refC = 1;
       array102.*.id = 102;
       array102.*.data = try allocator.alloc(u32, array102.*.size);
   }
   ...
   const array103 = try func17(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array106: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array106 = vars.data[pCounter];
          array106.refC += 1;
      } else {
          array106 = try allocator.create(Array);
          array106.*.size = 672;
          array106.*.refC = 1;
          array106.*.id = 106;
          array106.*.data = try allocator.alloc(u32, array106.*.size);
      }
      var loop32: usize = 0;
      const loopLimit32: usize = (50)/3 + 1;
      while (loop32 < loopLimit32) {
         for (0..array106.size) |i| {
             array106.data[i] -= 1;
         }
         for (0..array103.size) |i| {
      ...
      const array107 = try func28(&params1, try get_path());
      defer allocator.free(params1.data);
      array107.refC -= 1;
      if (array107.refC == 0) {
          allocator.free(array107.data);
          allocator.destroy(array107);
      }
      array106.refC -= 1;
      if (array106.refC == 0) {
          allocator.free(array106.data);
          allocator.destroy(array106);
      }
      ...
      const array108 = try func22(&params1, try get_path());
      defer allocator.free(params1.data);
      array108.refC -= 1;
      if (array108.refC == 0) {
          allocator.free(array108.data);
          allocator.destroy(array108);
      }
   }
   array103.refC -= 1;
   if (array103.refC == 0) {
       allocator.free(array103.data);
       allocator.destroy(array103);
   }
   return array102;
}
