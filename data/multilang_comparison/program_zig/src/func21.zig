...
pub fn func21(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   const path0 = _path0;
   _ = &path0;
   var array141: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array141 = vars.data[pCounter];
       array141.refC += 1;
   } else {
       array141 = try allocator.create(Array);
       array141.*.size = 43;
       array141.*.refC = 1;
       array141.*.id = 141;
       array141.*.data = try allocator.alloc(u32, array141.*.size);
   }
   ...
   const array142 = try func23(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array143: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array143 = vars.data[pCounter];
          array143.refC += 1;
      } else {
          array143 = try allocator.create(Array);
          array143.*.size = 428;
          array143.*.refC = 1;
          array143.*.id = 143;
          array143.*.data = try allocator.alloc(u32, array143.*.size);
      }
      var loop42: usize = 0;
      const loopLimit42: usize = (50)/3 + 1;
      while (loop42 < loopLimit42) {
         for (0..array141.size) |i| {
             array141.data[i] -= 1;
         }
         for (0..array143.size) |i| {
      ...
      const array144 = try func36(&params1, try get_path());
      ...
      array143.refC -= 1;
      if (array143.refC == 0) {
          allocator.free(array143.data);
          allocator.destroy(array143);
      }
      ...
      const array145 = try func32(&params1, try get_path());
      defer allocator.free(params1.data);
      array145.refC -= 1;
      if (array145.refC == 0) {
          allocator.free(array145.data);
          allocator.destroy(array145);
      }
   }
   array141.refC -= 1;
   if (array141.refC == 0) {
