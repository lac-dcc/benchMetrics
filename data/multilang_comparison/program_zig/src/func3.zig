...
pub fn func3(vars: *ArrayParam) !*Array {
   ...
   _ = &pCounter;
   var array195: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array195 = vars.data[pCounter];
       array195.refC += 1;
   } else {
       array195 = try allocator.create(Array);
       array195.*.size = 57;
       array195.*.refC = 1;
       array195.*.id = 195;
       array195.*.data = try allocator.alloc(u32, array195.*.size);
   }
   ...
   const array196 = try func6(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop56: usize = 0;
   const loopLimit56: usize = (50)/2 + 1;
   while (loop56 < loopLimit56) {
      for (0..array196.size) |i| {
          array196.data[i] += 1;
      }
      loop56 += 1;
   }
   for (0..array195.size) |i| {
