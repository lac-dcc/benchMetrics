...
pub fn func9(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   const path0 = _path0;
   _ = &path0;
   var array271: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array271 = vars.data[pCounter];
       array271.refC += 1;
   } else {
       array271 = try allocator.create(Array);
       array271.*.size = 338;
       array271.*.refC = 1;
       array271.*.id = 271;
       array271.*.data = try allocator.alloc(u32, array271.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array271;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array272 = try func15(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array275: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array275 = vars.data[pCounter];
          array275.refC += 1;
      } else {
          array275 = try allocator.create(Array);
          array275.*.size = 850;
          array275.*.refC = 1;
          array275.*.id = 275;
          array275.*.data = try allocator.alloc(u32, array275.*.size);
      }
      var loop79: usize = 0;
      const loopLimit79: usize = (50)/2 + 1;
      while (loop79 < loopLimit79) {
         for (0..array272.size) |i| {
             array272.data[i] -= 1;
         }
         for (0..array275.size) |i| {
             if (array275.data[i] == 54) {
                 return array275;
             }
         }
         loop79 += 1;
      ...
      params1_data[0] = array271;
      params1_data[1] = array272;
      params1_data[2] = array275;
      var params1 = ArrayParam{
          .data = params1_data,
          .size = 3,
      };
      const array276 = try func24(&params1, try get_path());
      defer allocator.free(params1.data);
      array276.refC -= 1;
      if (array276.refC == 0) {
          allocator.free(array276.data);
          allocator.destroy(array276);
      }
      array275.refC -= 1;
      if (array275.refC == 0) {
          allocator.free(array275.data);
          allocator.destroy(array275);
      }
      ...
      const array277 = try func16(&params1, try get_path());
      defer allocator.free(params1.data);
      array277.refC -= 1;
      if (array277.refC == 0) {
          allocator.free(array277.data);
          allocator.destroy(array277);
      }
   }
   array272.refC -= 1;
   if (array272.refC == 0) {
       allocator.free(array272.data);
       allocator.destroy(array272);
   }
   return array271;
}
