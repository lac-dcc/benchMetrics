...
pub fn func17(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array200: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array200 = vars.data[pCounter];
       array200.refC += 1;
   } else {
       array200 = try allocator.create(Array);
       array200.*.size = 516;
       array200.*.refC = 1;
       array200.*.id = 200;
       array200.*.data = try allocator.alloc(u32, array200.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array200;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array201 = try func24(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop58: usize = 0;
   const loopLimit58: usize = (50)/3 + 1;
   while (loop58 < loopLimit58) {
      for (0..array201.size) |i| {
          array201.data[i] += 1;
      }
      loop58 += 1;
   }
   for (0..array200.size) |i| {
       if (array200.data[i] == 4) {
           return array200;
       }
   }
   array201.refC -= 1;
   if (array201.refC == 0) {
       allocator.free(array201.data);
       allocator.destroy(array201);
   }
   return array200;
}
