...
pub fn func37(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   var loop40: usize = 0;
   const loopLimit40: usize = (50)/4 + 1;
   while (loop40 < loopLimit40) {
      if ((path0 & 1) != 0) {
         var array129: *Array = undefined;
         if (pCounter > 0) {
             pCounter -= 1;
             array129 = vars.data[pCounter];
             array129.refC += 1;
         } else {
             array129 = try allocator.create(Array);
             array129.*.size = 303;
             array129.*.refC = 1;
             array129.*.id = 129;
             array129.*.data = try allocator.alloc(u32, array129.*.size);
         }
         var loop41: usize = 0;
         const loopLimit41: usize = (50)/5 + 1;
         while (loop41 < loopLimit41) {
            for (0..array129.size) |i| {
                array129.data[i] -= 1;
            }
            for (0..array129.size) |i| {
                if (array129.data[i] == 97) {
                    return array129;
                }
            }
            loop41 += 1;
         ...
         const array130 = try func54(&params0, try get_path());
         defer allocator.free(params0.data);
         array130.refC -= 1;
         if (array130.refC == 0) {
             allocator.free(array130.data);
             allocator.destroy(array130);
         }
         array129.refC -= 1;
         if (array129.refC == 0) {
             allocator.free(array129.data);
             allocator.destroy(array129);
         }
         ...
         const array131 = try func50(&params0, try get_path());
         defer allocator.free(params0.data);
         array131.refC -= 1;
         if (array131.refC == 0) {
             allocator.free(array131.data);
             allocator.destroy(array131);
         }
      }
      var array132: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array132 = vars.data[pCounter];
          array132.refC += 1;
      } else {
          array132 = try allocator.create(Array);
          array132.*.size = 152;
          array132.*.refC = 1;
          array132.*.id = 132;
          array132.*.data = try allocator.alloc(u32, array132.*.size);
      }
      const params0_data = try allocator.alloc(*Array, 1);
      params0_data[0] = array132;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array133 = try func51(&params0);
      defer allocator.free(params0.data);
      if ((path0 & 2) != 0) {
         var array134: *Array = undefined;
         if (pCounter > 0) {
             pCounter -= 1;
             array134 = vars.data[pCounter];
             array134.refC += 1;
         } else {
             array134 = try allocator.create(Array);
             array134.*.size = 188;
             array134.*.refC = 1;
             array134.*.id = 134;
             array134.*.data = try allocator.alloc(u32, array134.*.size);
         }
         var loop42: usize = 0;
         const loopLimit42: usize = (50)/5 + 1;
         while (loop42 < loopLimit42) {
            for (0..array132.size) |i| {
                array132.data[i] -= 1;
            }
            for (0..array132.size) |i| {
                if (array132.data[i] == 36) {
                    return array132;
                }
            }
            loop42 += 1;
         ...
         params1_data[0] = array132;
         params1_data[1] = array133;
         params1_data[2] = array134;
         var params1 = ArrayParam{
             .data = params1_data,
             .size = 3,
         };
         const array135 = try func60(&params1, try get_path());
         defer allocator.free(params1.data);
         array135.refC -= 1;
         if (array135.refC == 0) {
             allocator.free(array135.data);
             allocator.destroy(array135);
         }
         array134.refC -= 1;
         if (array134.refC == 0) {
             allocator.free(array134.data);
             allocator.destroy(array134);
         }
         ...
         const array136 = try func52(&params1);
         defer allocator.free(params1.data);
         array136.refC -= 1;
         if (array136.refC == 0) {
             allocator.free(array136.data);
             allocator.destroy(array136);
         }
      }
      for (0..array132.size) |i| {
          if (array132.data[i] == 28) {
              return array132;
          }
      }
      array133.refC -= 1;
      if (array133.refC == 0) {
          allocator.free(array133.data);
          allocator.destroy(array133);
      }
      array132.refC -= 1;
      if (array132.refC == 0) {
          allocator.free(array132.data);
          allocator.destroy(array132);
      }
      loop40 += 1;
   }
   var array137: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array137 = vars.data[pCounter];
       array137.refC += 1;
   } else {
       array137 = try allocator.create(Array);
       array137.*.size = 27;
       array137.*.refC = 1;
       array137.*.id = 137;
       array137.*.data = try allocator.alloc(u32, array137.*.size);
   }
   return array137;
}
