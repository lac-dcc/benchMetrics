...
pub fn func13(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   var array198: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array198 = vars.data[pCounter];
       array198.refC += 1;
   } else {
       array198 = try allocator.create(Array);
       array198.*.size = 30;
       array198.*.refC = 1;
       array198.*.id = 198;
       array198.*.data = try allocator.alloc(u32, array198.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array198;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array199 = try func17(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array202: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array202 = vars.data[pCounter];
          array202.refC += 1;
      } else {
          array202 = try allocator.create(Array);
          array202.*.size = 580;
          array202.*.refC = 1;
          array202.*.id = 202;
          array202.*.data = try allocator.alloc(u32, array202.*.size);
      }
      var loop59: usize = 0;
      const loopLimit59: usize = (50)/3 + 1;
      while (loop59 < loopLimit59) {
         for (0..array198.size) |i| {
             array198.data[i] -= 1;
         }
         for (0..array198.size) |i| {
             if (array198.data[i] == 70) {
                 return array198;
             }
         }
         loop59 += 1;
      ...
      params1_data[0] = array198;
      params1_data[1] = array199;
      params1_data[2] = array202;
      var params1 = ArrayParam{
          .data = params1_data,
          .size = 3,
      };
      const array203 = try func28(&params1, try get_path());
      defer allocator.free(params1.data);
      array203.refC -= 1;
      if (array203.refC == 0) {
          allocator.free(array203.data);
          allocator.destroy(array203);
      }
      array202.refC -= 1;
      if (array202.refC == 0) {
          allocator.free(array202.data);
          allocator.destroy(array202);
      }
      ...
      const array204 = try func22(&params1, try get_path());
      defer allocator.free(params1.data);
      array204.refC -= 1;
      if (array204.refC == 0) {
          allocator.free(array204.data);
          allocator.destroy(array204);
      }
   }
   array198.refC -= 1;
   if (array198.refC == 0) {
