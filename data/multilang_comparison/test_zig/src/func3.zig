...
pub fn func3(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array291: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array291 = vars.data[pCounter];
       array291.refC += 1;
   } else {
       array291 = try allocator.create(Array);
       array291.*.size = 211;
       array291.*.refC = 1;
       array291.*.id = 291;
       array291.*.data = try allocator.alloc(u32, array291.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array291;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array292 = try func6(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop83: usize = 0;
   const loopLimit83: usize = (50)/2 + 1;
   while (loop83 < loopLimit83) {
      for (0..array292.size) |i| {
          array292.data[i] += 1;
      }
      loop83 += 1;
   }
   for (0..array291.size) |i| {
       if (array291.data[i] == 90) {
           return array291;
       }
   }
   array291.refC -= 1;
   if (array291.refC == 0) {
       allocator.free(array291.data);
       allocator.destroy(array291);
   }
   return array292;
}
