...
pub fn func12(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array161: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array161 = vars.data[pCounter];
          array161.refC += 1;
      } else {
          array161 = try allocator.create(Array);
          array161.*.size = 614;
          array161.*.refC = 1;
          array161.*.id = 161;
          array161.*.data = try allocator.alloc(u32, array161.*.size);
      }
      var loop48: usize = 0;
      const loopLimit48: usize = (50)/3 + 1;
      while (loop48 < loopLimit48) {
         for (0..array161.size) |i| {
             array161.data[i] -= 1;
         }
         for (0..array161.size) |i| {
             if (array161.data[i] == 58) {
                 return array161;
             }
         }
         loop48 += 1;
      ...
      params0_data[0] = array161;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array162 = try func24(&params0, try get_path());
      ...
      const array186 = try func16(&params0, try get_path());
      defer allocator.free(params0.data);
      array186.refC -= 1;
      if (array186.refC == 0) {
   ...
   var array196: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array196 = vars.data[pCounter];
       array196.refC += 1;
   } else {
       array196 = try allocator.create(Array);
       array196.*.size = 951;
       array196.*.refC = 1;
       array196.*.id = 196;
       array196.*.data = try allocator.alloc(u32, array196.*.size);
   }
   return array196;
}
