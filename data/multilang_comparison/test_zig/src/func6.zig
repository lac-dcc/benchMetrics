...
pub fn func6(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   if ((path0 & 1) != 0) {
      ...
      const array255 = try func7(&params0, try get_path());
      defer allocator.free(params0.data);
      array255.refC -= 1;
      if (array255.refC == 0) {
          allocator.free(array255.data);
          allocator.destroy(array255);
      }
      ...
      const array265 = try func8(&params0, try get_path());
      defer allocator.free(params0.data);
      array265.refC -= 1;
      if (array265.refC == 0) {
          allocator.free(array265.data);
          allocator.destroy(array265);
      }
   }
   const params0_data = try allocator.alloc(*Array, 0);
   var params0 = ArrayParam{
   ...
   const array270 = try func9(&params0, try get_path());
   defer allocator.free(params0.data);
   return array270;
}
