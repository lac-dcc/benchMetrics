...
pub fn func20(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array232: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array232 = vars.data[pCounter];
          array232.refC += 1;
      } else {
          array232 = try allocator.create(Array);
          array232.*.size = 604;
          array232.*.refC = 1;
          array232.*.id = 232;
          array232.*.data = try allocator.alloc(u32, array232.*.size);
      }
      var loop68: usize = 0;
      const loopLimit68: usize = (50)/3 + 1;
      while (loop68 < loopLimit68) {
         for (0..array232.size) |i| {
             array232.data[i] -= 1;
         }
         for (0..array232.size) |i| {
             if (array232.data[i] == 50) {
                 return array232;
             }
         }
         loop68 += 1;
      ...
      params0_data[0] = array232;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array233 = try func28(&params0, try get_path());
      defer allocator.free(params0.data);
      array233.refC -= 1;
      if (array233.refC == 0) {
          allocator.free(array233.data);
          allocator.destroy(array233);
      }
      array232.refC -= 1;
      if (array232.refC == 0) {
          allocator.free(array232.data);
          allocator.destroy(array232);
      }
      ...
      const array234 = try func22(&params0, try get_path());
      defer allocator.free(params0.data);
      array234.refC -= 1;
      if (array234.refC == 0) {
          allocator.free(array234.data);
          allocator.destroy(array234);
      }
   }
   var array235: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array235 = vars.data[pCounter];
       array235.refC += 1;
   } else {
       array235 = try allocator.create(Array);
       array235.*.size = 195;
       array235.*.refC = 1;
       array235.*.id = 235;
       array235.*.data = try allocator.alloc(u32, array235.*.size);
   }
   return array235;
}
