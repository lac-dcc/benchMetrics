...
pub fn func38(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array139: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array139 = vars.data[pCounter];
          array139.refC += 1;
      } else {
          array139 = try allocator.create(Array);
          array139.*.size = 697;
          array139.*.refC = 1;
          array139.*.id = 139;
          array139.*.data = try allocator.alloc(u32, array139.*.size);
      }
      var loop43: usize = 0;
      const loopLimit43: usize = (50)/4 + 1;
      while (loop43 < loopLimit43) {
         for (0..array139.size) |i| {
             array139.data[i] -= 1;
         }
         for (0..array139.size) |i| {
             if (array139.data[i] == 39) {
                 return array139;
             }
         }
         loop43 += 1;
      ...
      params0_data[0] = array139;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array140 = try func46(&params0, try get_path());
      defer allocator.free(params0.data);
      array140.refC -= 1;
      if (array140.refC == 0) {
          allocator.free(array140.data);
          allocator.destroy(array140);
      }
      array139.refC -= 1;
      if (array139.refC == 0) {
          allocator.free(array139.data);
          allocator.destroy(array139);
      }
      ...
      const array141 = try func40(&params0, try get_path());
      defer allocator.free(params0.data);
      array141.refC -= 1;
      if (array141.refC == 0) {
          allocator.free(array141.data);
          allocator.destroy(array141);
      }
   }
   var array142: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array142 = vars.data[pCounter];
       array142.refC += 1;
   } else {
       array142 = try allocator.create(Array);
       array142.*.size = 681;
       array142.*.refC = 1;
       array142.*.id = 142;
       array142.*.data = try allocator.alloc(u32, array142.*.size);
   }
   return array142;
}
