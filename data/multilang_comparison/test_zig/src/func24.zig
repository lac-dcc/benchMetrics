...
pub fn func24(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   if ((path0 & 1) != 0) {
      ...
      const array163 = try func25(&params0, try get_path());
      ...
      const array173 = try func26(&params0, try get_path());
      defer allocator.free(params0.data);
      array173.refC -= 1;
      if (array173.refC == 0) {
          allocator.free(array173.data);
          allocator.destroy(array173);
      }
   }
   const params0_data = try allocator.alloc(*Array, 0);
   var params0 = ArrayParam{
   ...
   const array178 = try func27(&params0, try get_path());
   defer allocator.free(params0.data);
   return array178;
}
