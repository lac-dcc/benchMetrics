...
pub fn func45(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   var array86: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array86 = vars.data[pCounter];
       array86.refC += 1;
   } else {
       array86 = try allocator.create(Array);
       array86.*.size = 813;
       array86.*.refC = 1;
       array86.*.id = 86;
       array86.*.data = try allocator.alloc(u32, array86.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array86;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array87 = try func51(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array90: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array90 = vars.data[pCounter];
          array90.refC += 1;
      } else {
          array90 = try allocator.create(Array);
          array90.*.size = 936;
          array90.*.refC = 1;
          array90.*.id = 90;
          array90.*.data = try allocator.alloc(u32, array90.*.size);
      }
      var loop29: usize = 0;
      ...
      while (loop29 < loopLimit29) {
         ...
         for (0..array86.size) |i| {
             if (array86.data[i] == 56) {
                 return array86;
             }
         }
         loop29 += 1;
      ...
      params1_data[0] = array86;
      params1_data[1] = array87;
      params1_data[2] = array90;
      var params1 = ArrayParam{
          .data = params1_data,
          .size = 3,
      };
      const array91 = try func60(&params1, try get_path());
      defer allocator.free(params1.data);
      array91.refC -= 1;
      if (array91.refC == 0) {
          allocator.free(array91.data);
          allocator.destroy(array91);
      }
      array90.refC -= 1;
      if (array90.refC == 0) {
          allocator.free(array90.data);
          allocator.destroy(array90);
      }
      ...
      const array92 = try func52(&params1);
      defer allocator.free(params1.data);
      array92.refC -= 1;
      if (array92.refC == 0) {
