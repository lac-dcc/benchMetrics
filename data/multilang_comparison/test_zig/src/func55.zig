...
pub fn func55(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var loop19: usize = 0;
   const loopLimit19: usize = (50)/5 + 1;
   while (loop19 < loopLimit19) {
      loop19 += 1;
   }
   var array53: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array53 = vars.data[pCounter];
       array53.refC += 1;
   } else {
       array53 = try allocator.create(Array);
       array53.*.size = 29;
       array53.*.refC = 1;
       array53.*.id = 53;
       array53.*.data = try allocator.alloc(u32, array53.*.size);
   }
   return array53;
}
