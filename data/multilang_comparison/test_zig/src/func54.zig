...
pub fn func54(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   if ((path0 & 1) != 0) {
      ...
      const array52 = try func55(&params0);
      ...
      const array54 = try func56(&params0, try get_path());
      defer allocator.free(params0.data);
      array54.refC -= 1;
      if (array54.refC == 0) {
          allocator.free(array54.data);
          allocator.destroy(array54);
      }
   }
   const params0_data = try allocator.alloc(*Array, 0);
   var params0 = ArrayParam{
   ...
   const array58 = try func57(&params0);
   defer allocator.free(params0.data);
   return array58;
}
