...
pub fn func41(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array48: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array48 = vars.data[pCounter];
       array48.refC += 1;
   } else {
       array48 = try allocator.create(Array);
       array48.*.size = 914;
       array48.*.refC = 1;
       array48.*.id = 48;
       array48.*.data = try allocator.alloc(u32, array48.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array48;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array49 = try func46(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop17: usize = 0;
   const loopLimit17: usize = (50)/5 + 1;
   while (loop17 < loopLimit17) {
      for (0..array49.size) |i| {
          array49.data[i] += 1;
      }
      loop17 += 1;
   }
