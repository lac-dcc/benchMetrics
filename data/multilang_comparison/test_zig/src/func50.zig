...
pub fn func50(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   var loop21: usize = 0;
   const loopLimit21: usize = (50)/5 + 1;
   while (loop21 < loopLimit21) {
      if ((path0 & 1) != 0) {
         var array63: *Array = undefined;
         if (pCounter > 0) {
             pCounter -= 1;
             array63 = vars.data[pCounter];
             array63.refC += 1;
         } else {
             array63 = try allocator.create(Array);
             array63.*.size = 470;
             array63.*.refC = 1;
             array63.*.id = 63;
             array63.*.data = try allocator.alloc(u32, array63.*.size);
         }
         var loop22: usize = 0;
         const loopLimit22: usize = (50)/6 + 1;
         while (loop22 < loopLimit22) {
            for (0..array63.size) |i| {
                array63.data[i] -= 1;
            }
            for (0..array62.size) |i| {
                if (array62.data[i] == 99) {
                    return array62;
                }
            }
            loop22 += 1;
         ...
         params0_data[0] = array62;
         var params0 = ArrayParam{
             .data = params0_data,
             .size = 1,
         };
         const array64 = try func58(&params0);
         defer allocator.free(params0.data);
         array64.refC -= 1;
         if (array64.refC == 0) {
             allocator.free(array64.data);
             allocator.destroy(array64);
         }
      }
      var array65: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array65 = vars.data[pCounter];
          array65.refC += 1;
      } else {
          array65 = try allocator.create(Array);
          array65.*.size = 644;
          array65.*.refC = 1;
          array65.*.id = 65;
          array65.*.data = try allocator.alloc(u32, array65.*.size);
      }
      const params0_data = try allocator.alloc(*Array, 2);
      params0_data[0] = array62;
      params0_data[1] = array65;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 2,
      };
      const array66 = try func59(&params0);
      defer allocator.free(params0.data);
      for (0..array66.size) |i| {
          if (array66.data[i] == 54) {
              return array66;
          }
      }
      array66.refC -= 1;
      if (array66.refC == 0) {
          allocator.free(array66.data);
          allocator.destroy(array66);
      }
      array65.refC -= 1;
      if (array65.refC == 0) {
          allocator.free(array65.data);
          allocator.destroy(array65);
      }
      loop21 += 1;
   }
   for (0..array62.size) |i| {
