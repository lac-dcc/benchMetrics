...
pub fn func1(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   const path0 = _path0;
   _ = &path0;
   var array289: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array289 = vars.data[pCounter];
       array289.refC += 1;
   } else {
       array289 = try allocator.create(Array);
       array289.*.size = 21;
       array289.*.refC = 1;
       array289.*.id = 289;
       array289.*.data = try allocator.alloc(u32, array289.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array289;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array290 = try func3(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array293: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array293 = vars.data[pCounter];
          array293.refC += 1;
      } else {
          array293 = try allocator.create(Array);
          array293.*.size = 314;
          array293.*.refC = 1;
          array293.*.id = 293;
          array293.*.data = try allocator.alloc(u32, array293.*.size);
      }
      var loop84: usize = 0;
      const loopLimit84: usize = (50)/2 + 1;
      while (loop84 < loopLimit84) {
         for (0..array290.size) |i| {
             array290.data[i] -= 1;
         }
         for (0..array290.size) |i| {
             if (array290.data[i] == 20) {
                 return array290;
             }
         }
         loop84 += 1;
      ...
      params1_data[0] = array289;
      params1_data[1] = array290;
      params1_data[2] = array293;
      var params1 = ArrayParam{
          .data = params1_data,
          .size = 3,
      };
      const array294 = try func10(&params1, try get_path());
      defer allocator.free(params1.data);
      array294.refC -= 1;
      if (array294.refC == 0) {
          allocator.free(array294.data);
          allocator.destroy(array294);
      }
      array293.refC -= 1;
      if (array293.refC == 0) {
          allocator.free(array293.data);
          allocator.destroy(array293);
      }
      ...
      const array295 = try func4(&params1, try get_path());
      defer allocator.free(params1.data);
      array295.refC -= 1;
      if (array295.refC == 0) {
          allocator.free(array295.data);
          allocator.destroy(array295);
      }
   }
   array289.refC -= 1;
   if (array289.refC == 0) {
       allocator.free(array289.data);
       allocator.destroy(array289);
   }
   return array290;
}
