...
pub fn func49(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   var array31: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array31 = vars.data[pCounter];
       array31.refC += 1;
   } else {
       array31 = try allocator.create(Array);
       array31.*.size = 444;
       array31.*.refC = 1;
       array31.*.id = 31;
       array31.*.data = try allocator.alloc(u32, array31.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array31;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array32 = try func53(&params0);
   ...
   if ((path0 & 1) != 0) {
      ...
      array35.refC -= 1;
      if (array35.refC == 0) {
          allocator.free(array35.data);
          allocator.destroy(array35);
      }
   
   } else {
      const params1_data = try allocator.alloc(*Array, 2);
      params1_data[0] = array31;
      params1_data[1] = array32;
      var params1 = ArrayParam{
          .data = params1_data,
          .size = 2,
      };
      const array36 = try func58(&params1);
