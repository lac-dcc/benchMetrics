...
pub fn func48(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array19: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array19 = vars.data[pCounter];
          array19.refC += 1;
      } else {
          array19 = try allocator.create(Array);
          array19.*.size = 814;
          array19.*.refC = 1;
          array19.*.id = 19;
          array19.*.data = try allocator.alloc(u32, array19.*.size);
      }
      var loop10: usize = 0;
      const loopLimit10: usize = (50)/5 + 1;
      while (loop10 < loopLimit10) {
         for (0..array19.size) |i| {
             array19.data[i] -= 1;
         }
         for (0..array19.size) |i| {
             if (array19.data[i] == 64) {
                 return array19;
             }
         }
         loop10 += 1;
      ...
      params0_data[0] = array19;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array20 = try func60(&params0, try get_path());
      defer allocator.free(params0.data);
      array20.refC -= 1;
      if (array20.refC == 0) {
          allocator.free(array20.data);
          allocator.destroy(array20);
      }
      array19.refC -= 1;
      if (array19.refC == 0) {
          allocator.free(array19.data);
          allocator.destroy(array19);
      }
      ...
      const array27 = try func52(&params0);
      defer allocator.free(params0.data);
      array27.refC -= 1;
      if (array27.refC == 0) {
          allocator.free(array27.data);
          allocator.destroy(array27);
      }
   }
   var array29: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array29 = vars.data[pCounter];
       array29.refC += 1;
   } else {
       array29 = try allocator.create(Array);
       array29.*.size = 301;
       array29.*.refC = 1;
       array29.*.id = 29;
       array29.*.data = try allocator.alloc(u32, array29.*.size);
   }
   return array29;
}
