...
pub fn func26(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array174: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array174 = vars.data[pCounter];
          array174.refC += 1;
      } else {
          array174 = try allocator.create(Array);
          array174.*.size = 433;
          array174.*.refC = 1;
          array174.*.id = 174;
          array174.*.data = try allocator.alloc(u32, array174.*.size);
      }
      var loop52: usize = 0;
      const loopLimit52: usize = (50)/3 + 1;
      while (loop52 < loopLimit52) {
         for (0..array174.size) |i| {
             array174.data[i] -= 1;
         }
         for (0..array174.size) |i| {
             if (array174.data[i] == 38) {
                 return array174;
             }
         }
         loop52 += 1;
      ...
      params0_data[0] = array174;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array175 = try func36(&params0, try get_path());
      ...
      array174.refC -= 1;
      if (array174.refC == 0) {
          allocator.free(array174.data);
          allocator.destroy(array174);
      }
      ...
      const array176 = try func32(&params0, try get_path());
      defer allocator.free(params0.data);
      array176.refC -= 1;
      if (array176.refC == 0) {
          allocator.free(array176.data);
          allocator.destroy(array176);
      }
   }
   var array177: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array177 = vars.data[pCounter];
       array177.refC += 1;
   } else {
       array177 = try allocator.create(Array);
       array177.*.size = 417;
       array177.*.refC = 1;
       array177.*.id = 177;
       array177.*.data = try allocator.alloc(u32, array177.*.size);
   }
   return array177;
}
