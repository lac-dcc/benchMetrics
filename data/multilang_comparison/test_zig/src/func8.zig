...
pub fn func8(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array266: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array266 = vars.data[pCounter];
          array266.refC += 1;
      } else {
          array266 = try allocator.create(Array);
          array266.*.size = 189;
          array266.*.refC = 1;
          array266.*.id = 266;
          array266.*.data = try allocator.alloc(u32, array266.*.size);
      }
      var loop77: usize = 0;
      const loopLimit77: usize = (50)/2 + 1;
      while (loop77 < loopLimit77) {
         for (0..array266.size) |i| {
             array266.data[i] -= 1;
         }
         for (0..array266.size) |i| {
             if (array266.data[i] == 10) {
                 return array266;
             }
         }
         loop77 += 1;
      ...
      params0_data[0] = array266;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array267 = try func18(&params0, try get_path());
      defer allocator.free(params0.data);
      array267.refC -= 1;
      if (array267.refC == 0) {
          allocator.free(array267.data);
          allocator.destroy(array267);
      }
      array266.refC -= 1;
      if (array266.refC == 0) {
          allocator.free(array266.data);
          allocator.destroy(array266);
      }
      ...
      const array268 = try func14(&params0, try get_path());
      defer allocator.free(params0.data);
      array268.refC -= 1;
      if (array268.refC == 0) {
          allocator.free(array268.data);
          allocator.destroy(array268);
      }
   }
   var array269: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array269 = vars.data[pCounter];
       array269.refC += 1;
   } else {
       array269 = try allocator.create(Array);
       array269.*.size = 0;
       array269.*.refC = 1;
       array269.*.id = 269;
       array269.*.data = try allocator.alloc(u32, array269.*.size);
   }
   return array269;
}
