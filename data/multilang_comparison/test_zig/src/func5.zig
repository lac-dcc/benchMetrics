...
pub fn func5(vars: *ArrayParam) !*Array {
   ...
   _ = &pCounter;
   var array217: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array217 = vars.data[pCounter];
       array217.refC += 1;
   } else {
       array217 = try allocator.create(Array);
       array217.*.size = 93;
       array217.*.refC = 1;
       array217.*.id = 217;
       array217.*.data = try allocator.alloc(u32, array217.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array217;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array218 = try func10(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop63: usize = 0;
   const loopLimit63: usize = (50)/3 + 1;
   while (loop63 < loopLimit63) {
      for (0..array218.size) |i| {
          array218.data[i] += 1;
      }
      loop63 += 1;
   }
   for (0..array217.size) |i| {
       if (array217.data[i] == 53) {
           return array217;
       }
   }
   array217.refC -= 1;
   if (array217.refC == 0) {
       allocator.free(array217.data);
       allocator.destroy(array217);
   }
   return array218;
}
