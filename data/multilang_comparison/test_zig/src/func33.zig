...
pub fn func33(vars: *ArrayParam) !*Array {
   ...
   _ = &pCounter;
   var array181: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array181 = vars.data[pCounter];
       array181.refC += 1;
   } else {
       array181 = try allocator.create(Array);
       array181.*.size = 873;
       array181.*.refC = 1;
       array181.*.id = 181;
       array181.*.data = try allocator.alloc(u32, array181.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array181;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array182 = try func36(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop53: usize = 0;
   const loopLimit53: usize = (50)/3 + 1;
   while (loop53 < loopLimit53) {
      for (0..array182.size) |i| {
          array182.data[i] += 1;
      }
      loop53 += 1;
   }
   for (0..array182.size) |i| {
       if (array182.data[i] == 83) {
           return array182;
       }
   }
   array181.refC -= 1;
