...
pub fn func35(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array107: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array107 = vars.data[pCounter];
       array107.refC += 1;
   } else {
       array107 = try allocator.create(Array);
       array107.*.size = 299;
       array107.*.refC = 1;
       array107.*.id = 107;
       array107.*.data = try allocator.alloc(u32, array107.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array107;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array108 = try func42(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop33: usize = 0;
   const loopLimit33: usize = (50)/4 + 1;
   while (loop33 < loopLimit33) {
      for (0..array107.size) |i| {
          array107.data[i] += 1;
      }
      loop33 += 1;
   }
   for (0..array108.size) |i| {
       if (array108.data[i] == 98) {
           return array108;
       }
   }
   array108.refC -= 1;
