...
pub fn func51(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array88: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array88 = vars.data[pCounter];
       array88.refC += 1;
   } else {
       array88 = try allocator.create(Array);
       array88.*.size = 466;
       array88.*.refC = 1;
       array88.*.id = 88;
       array88.*.data = try allocator.alloc(u32, array88.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array88;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array89 = try func54(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop28: usize = 0;
   const loopLimit28: usize = (50)/4 + 1;
   while (loop28 < loopLimit28) {
      for (0..array88.size) |i| {
          array88.data[i] += 1;
      }
      loop28 += 1;
   }
   for (0..array88.size) |i| {
       if (array88.data[i] == 76) {
           return array88;
       }
   }
   array89.refC -= 1;
