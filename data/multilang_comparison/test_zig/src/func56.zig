...
pub fn func56(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   if ((path0 & 1) != 0) {
      var array55: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array55 = vars.data[pCounter];
          array55.refC += 1;
      } else {
          array55 = try allocator.create(Array);
          array55.*.size = 143;
          array55.*.refC = 1;
          array55.*.id = 55;
          array55.*.data = try allocator.alloc(u32, array55.*.size);
      }
      var loop20: usize = 0;
      const loopLimit20: usize = (50)/5 + 1;
      while (loop20 < loopLimit20) {
         for (0..array55.size) |i| {
             array55.data[i] -= 1;
         }
         for (0..array55.size) |i| {
             if (array55.data[i] == 29) {
                 return array55;
             }
         }
         loop20 += 1;
      ...
      array55.refC -= 1;
      if (array55.refC == 0) {
          allocator.free(array55.data);
          allocator.destroy(array55);
      }
      ...
      const array56 = try func58(&params0);
      defer allocator.free(params0.data);
      array56.refC -= 1;
      if (array56.refC == 0) {
          allocator.free(array56.data);
          allocator.destroy(array56);
      }
   }
   var array57: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array57 = vars.data[pCounter];
       array57.refC += 1;
   } else {
       array57 = try allocator.create(Array);
       array57.*.size = 443;
       array57.*.refC = 1;
       array57.*.id = 57;
       array57.*.data = try allocator.alloc(u32, array57.*.size);
   }
   return array57;
}
