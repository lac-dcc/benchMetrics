...
pub fn func31(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   var array105: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array105 = vars.data[pCounter];
       array105.refC += 1;
   } else {
       array105 = try allocator.create(Array);
       array105.*.size = 672;
       array105.*.refC = 1;
       array105.*.id = 105;
       array105.*.data = try allocator.alloc(u32, array105.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array105;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array106 = try func35(&params0);
   defer allocator.free(params0.data);
   if ((path0 & 1) != 0) {
      var array109: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array109 = vars.data[pCounter];
          array109.refC += 1;
      } else {
          array109 = try allocator.create(Array);
          array109.*.size = 524;
          array109.*.refC = 1;
          array109.*.id = 109;
          array109.*.data = try allocator.alloc(u32, array109.*.size);
      }
      var loop34: usize = 0;
      const loopLimit34: usize = (50)/4 + 1;
      while (loop34 < loopLimit34) {
         for (0..array109.size) |i| {
             array109.data[i] -= 1;
         }
         for (0..array109.size) |i| {
             if (array109.data[i] == 81) {
                 return array109;
             }
         }
         loop34 += 1;
      ...
      params1_data[0] = array105;
      params1_data[1] = array106;
      params1_data[2] = array109;
      var params1 = ArrayParam{
          .data = params1_data,
          .size = 3,
      };
      const array110 = try func46(&params1, try get_path());
      defer allocator.free(params1.data);
      array110.refC -= 1;
      if (array110.refC == 0) {
          allocator.free(array110.data);
          allocator.destroy(array110);
      }
      array109.refC -= 1;
      if (array109.refC == 0) {
      ...
      const array111 = try func40(&params1, try get_path());
      defer allocator.free(params1.data);
      array111.refC -= 1;
      if (array111.refC == 0) {
