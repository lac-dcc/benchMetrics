...
pub fn func18(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   if ((path0 & 1) != 0) {
      ...
      const array221 = try func19(&params0, try get_path());
      defer allocator.free(params0.data);
      array221.refC -= 1;
      if (array221.refC == 0) {
          allocator.free(array221.data);
          allocator.destroy(array221);
      }
      ...
      const array231 = try func20(&params0, try get_path());
      defer allocator.free(params0.data);
      array231.refC -= 1;
      if (array231.refC == 0) {
          allocator.free(array231.data);
          allocator.destroy(array231);
      }
   }
   const params0_data = try allocator.alloc(*Array, 0);
   var params0 = ArrayParam{
   ...
   const array236 = try func21(&params0, try get_path());
   defer allocator.free(params0.data);
   return array236;
}
