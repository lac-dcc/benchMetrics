...
pub fn func0(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   while (loop1 < loopLimit1) {
      if ((path0 & 1) != 0) {
         ...
         const array205 = try func4(&params0, try get_path());
         defer allocator.free(params0.data);
         array205.refC -= 1;
         if (array205.refC == 0) {
             allocator.free(array205.data);
             allocator.destroy(array205);
         }
      }
      var array215: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array215 = vars.data[pCounter];
          array215.refC += 1;
      } else {
          array215 = try allocator.create(Array);
          array215.*.size = 882;
          array215.*.refC = 1;
          array215.*.id = 215;
          array215.*.data = try allocator.alloc(u32, array215.*.size);
      }
      const params0_data = try allocator.alloc(*Array, 2);
      params0_data[0] = array1;
      params0_data[1] = array215;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 2,
      };
      const array216 = try func5(&params0);
      defer allocator.free(params0.data);
      if ((path0 & 2) != 0) {
         var array219: *Array = undefined;
         if (pCounter > 0) {
             pCounter -= 1;
             array219 = vars.data[pCounter];
             array219.refC += 1;
         } else {
             array219 = try allocator.create(Array);
             array219.*.size = 74;
             array219.*.refC = 1;
             array219.*.id = 219;
             array219.*.data = try allocator.alloc(u32, array219.*.size);
         }
         var loop64: usize = 0;
         const loopLimit64: usize = (50)/3 + 1;
         while (loop64 < loopLimit64) {
            for (0..array216.size) |i| {
                array216.data[i] -= 1;
            }
            for (0..array215.size) |i| {
                if (array215.data[i] == 79) {
                    return array215;
                }
            }
            loop64 += 1;
         ...
         params1_data[1] = array215;
         params1_data[2] = array216;
         params1_data[3] = array219;
         var params1 = ArrayParam{
             .data = params1_data,
             .size = 4,
         };
         const array220 = try func18(&params1, try get_path());
         defer allocator.free(params1.data);
         array220.refC -= 1;
         if (array220.refC == 0) {
             allocator.free(array220.data);
             allocator.destroy(array220);
         }
         array219.refC -= 1;
         if (array219.refC == 0) {
             allocator.free(array219.data);
             allocator.destroy(array219);
         }
         ...
         const array242 = try func14(&params1, try get_path());
         defer allocator.free(params1.data);
         array242.refC -= 1;
         if (array242.refC == 0) {
             allocator.free(array242.data);
             allocator.destroy(array242);
         }
      }
      for (0..array1.size) |i| {
          if (array1.data[i] == 40) {
      ...
      array216.refC -= 1;
      if (array216.refC == 0) {
          allocator.free(array216.data);
          allocator.destroy(array216);
      }
      array215.refC -= 1;
      if (array215.refC == 0) {
          allocator.free(array215.data);
          allocator.destroy(array215);
      }
      loop1 += 1;
   }
   return array1;
