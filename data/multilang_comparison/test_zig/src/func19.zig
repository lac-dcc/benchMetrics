...
pub fn func19(vars: *ArrayParam, _path0: u64) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   const path0 = _path0;
   _ = &path0;
   var loop65: usize = 0;
   const loopLimit65: usize = (50)/3 + 1;
   while (loop65 < loopLimit65) {
      if ((path0 & 1) != 0) {
         var array222: *Array = undefined;
         if (pCounter > 0) {
             pCounter -= 1;
             array222 = vars.data[pCounter];
             array222.refC += 1;
         } else {
             array222 = try allocator.create(Array);
             array222.*.size = 17;
             array222.*.refC = 1;
             array222.*.id = 222;
             array222.*.data = try allocator.alloc(u32, array222.*.size);
         }
         var loop66: usize = 0;
         const loopLimit66: usize = (50)/4 + 1;
         while (loop66 < loopLimit66) {
            for (0..array222.size) |i| {
                array222.data[i] -= 1;
            }
            for (0..array222.size) |i| {
                if (array222.data[i] == 37) {
                    return array222;
                }
            }
            loop66 += 1;
         ...
         params0_data[0] = array222;
         var params0 = ArrayParam{
             .data = params0_data,
             .size = 1,
         };
         const array223 = try func36(&params0, try get_path());
         defer allocator.free(params0.data);
         array223.refC -= 1;
         if (array223.refC == 0) {
             allocator.free(array223.data);
             allocator.destroy(array223);
         }
         array222.refC -= 1;
         if (array222.refC == 0) {
             allocator.free(array222.data);
             allocator.destroy(array222);
         }
         ...
         const array224 = try func32(&params0, try get_path());
         defer allocator.free(params0.data);
         array224.refC -= 1;
         if (array224.refC == 0) {
             allocator.free(array224.data);
             allocator.destroy(array224);
         }
      }
      var array225: *Array = undefined;
      if (pCounter > 0) {
          pCounter -= 1;
          array225 = vars.data[pCounter];
          array225.refC += 1;
      } else {
          array225 = try allocator.create(Array);
          array225.*.size = 328;
          array225.*.refC = 1;
          array225.*.id = 225;
          array225.*.data = try allocator.alloc(u32, array225.*.size);
      }
      const params0_data = try allocator.alloc(*Array, 1);
      params0_data[0] = array225;
      var params0 = ArrayParam{
          .data = params0_data,
          .size = 1,
      };
      const array226 = try func33(&params0);
      defer allocator.free(params0.data);
      if ((path0 & 2) != 0) {
         var array227: *Array = undefined;
         if (pCounter > 0) {
             pCounter -= 1;
             array227 = vars.data[pCounter];
             array227.refC += 1;
         } else {
             array227 = try allocator.create(Array);
             array227.*.size = 710;
             array227.*.refC = 1;
             array227.*.id = 227;
             array227.*.data = try allocator.alloc(u32, array227.*.size);
         }
         var loop67: usize = 0;
         const loopLimit67: usize = (50)/4 + 1;
         while (loop67 < loopLimit67) {
            for (0..array225.size) |i| {
                array225.data[i] -= 1;
            }
            for (0..array226.size) |i| {
                if (array226.data[i] == 36) {
                    return array226;
                }
            }
            loop67 += 1;
         ...
         params1_data[0] = array225;
         params1_data[1] = array226;
         params1_data[2] = array227;
         var params1 = ArrayParam{
             .data = params1_data,
             .size = 3,
         };
         const array228 = try func42(&params1, try get_path());
         defer allocator.free(params1.data);
         array228.refC -= 1;
         if (array228.refC == 0) {
             allocator.free(array228.data);
             allocator.destroy(array228);
         }
         array227.refC -= 1;
         if (array227.refC == 0) {
             allocator.free(array227.data);
             allocator.destroy(array227);
         }
         ...
         const array229 = try func34(&params1, try get_path());
         defer allocator.free(params1.data);
         array229.refC -= 1;
         if (array229.refC == 0) {
             allocator.free(array229.data);
             allocator.destroy(array229);
         }
      }
      for (0..array225.size) |i| {
          if (array225.data[i] == 4) {
              return array225;
          }
      }
      array226.refC -= 1;
      if (array226.refC == 0) {
          allocator.free(array226.data);
          allocator.destroy(array226);
      }
      array225.refC -= 1;
      if (array225.refC == 0) {
          allocator.free(array225.data);
          allocator.destroy(array225);
      }
      loop65 += 1;
   }
   var array230: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array230 = vars.data[pCounter];
       array230.refC += 1;
   } else {
       array230 = try allocator.create(Array);
       array230.*.size = 571;
       array230.*.refC = 1;
       array230.*.id = 230;
       array230.*.data = try allocator.alloc(u32, array230.*.size);
   }
   return array230;
}
