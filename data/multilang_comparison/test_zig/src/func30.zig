...
pub fn func30(vars: *ArrayParam, _path0: u64) !*Array {
   ...
   if ((path0 & 1) != 0) {
      ...
      const array93 = try func34(&params0, try get_path());
      defer allocator.free(params0.data);
      array93.refC -= 1;
      if (array93.refC == 0) {
   ...
   var array103: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array103 = vars.data[pCounter];
       array103.refC += 1;
   } else {
       array103 = try allocator.create(Array);
       array103.*.size = 396;
       array103.*.refC = 1;
       array103.*.id = 103;
       array103.*.data = try allocator.alloc(u32, array103.*.size);
   }
   return array103;
}
