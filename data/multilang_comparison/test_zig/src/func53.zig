...
pub fn func53(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array33: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array33 = vars.data[pCounter];
       array33.refC += 1;
   } else {
       array33 = try allocator.create(Array);
       array33.*.size = 117;
       array33.*.refC = 1;
       array33.*.id = 33;
       array33.*.data = try allocator.alloc(u32, array33.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array33;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array34 = try func60(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop12: usize = 0;
   const loopLimit12: usize = (50)/5 + 1;
   while (loop12 < loopLimit12) {
      for (0..array34.size) |i| {
          array34.data[i] += 1;
      }
      loop12 += 1;
   }
