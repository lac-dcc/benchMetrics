...
pub fn func58(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array13: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array13 = vars.data[pCounter];
       array13.refC += 1;
   } else {
       array13 = try allocator.create(Array);
       array13.*.size = 526;
       array13.*.refC = 1;
       array13.*.id = 13;
       array13.*.data = try allocator.alloc(u32, array13.*.size);
   }
   for (0..array13.size) |i| {
       array13.data[i] -= 1;
   }
   for (0..array13.size) |i| {
       if (array13.data[i] == 56) {
           return array13;
       }
   }
   return array13;
