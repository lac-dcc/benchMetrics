...
pub fn func23(vars: *ArrayParam) !*Array {
   var pCounter: usize = vars.size;
   _ = &pCounter;
   var array124: *Array = undefined;
   if (pCounter > 0) {
       pCounter -= 1;
       array124 = vars.data[pCounter];
       array124.refC += 1;
   } else {
       array124 = try allocator.create(Array);
       array124.*.size = 808;
       array124.*.refC = 1;
       array124.*.id = 124;
       array124.*.data = try allocator.alloc(u32, array124.*.size);
   }
   const params0_data = try allocator.alloc(*Array, 1);
   params0_data[0] = array124;
   var params0 = ArrayParam{
       .data = params0_data,
       .size = 1,
   };
   const array125 = try func28(&params0, try get_path());
   defer allocator.free(params0.data);
   var loop38: usize = 0;
   const loopLimit38: usize = (50)/4 + 1;
   while (loop38 < loopLimit38) {
      for (0..array125.size) |i| {
          array125.data[i] += 1;
      }
      loop38 += 1;
   }
   for (0..array125.size) |i| {
       if (array125.data[i] == 33) {
           return array125;
       }
   }
   array125.refC -= 1;
   if (array125.refC == 0) {
       allocator.free(array125.data);
       allocator.destroy(array125);
   }
   return array124;
}
